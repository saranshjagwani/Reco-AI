...
export const saveTranscriptToBackend = async (transcript, email) => {
  try {
    const response = await fetch("http://localhost:5000/api/transcripts/save", {
      method: "POST",
...
const BASE_URL = "http://localhost:5000/api/transcripts"; // ✅ Update if needed
...
export const fetchTranscripts = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:5000/api/transcripts/get?email=${userEmail}`);
...
export const fetchTranscriptById = async (id) => {
  try {
    const response = await fetch(`http://localhost:5000/api/transcripts/get/${id}`);
    if (!response.ok) {
...
export const uploadAudio = async (audioFile) => {
  try {
    ...
    const response = await axios.post("http://localhost:5000/api/transcripts/upload-audio", formData, {
